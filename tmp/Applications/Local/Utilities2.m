...
line={{vx[[1]],vx[[4]]},{vx[[4]],vx[[6]]},{vx[\[PartialD][6]],vx[[3]]},
...
(* ::Input::Initialization:: *)
(*CreatePalette[
{PasteButton["CG[\"\[FilledSmallSquare]\"]",Defer[CG["\[SelectionPlaceholder]"]]],
PasteButton["CG[\[FilledSmallSquare]]",Defer[CG[\[SelectionPlaceholder]]]],
PasteButton["\[FilledSmallSquare][CG]",\[SelectionPlaceholder][Defer[CG["\[Placeholder]"]]]],
PasteButton["ColForm",Defer[ColumnForms[#,2]&]],
PasteButton["T",Defer[T[\[SelectionPlaceholder],"\[Placeholder]",{\[Placeholder]}]]]
}
];*)
(*CreatePalette[
{PasteButton["CG[\"\[FilledSmallSquare]\"]",Defer[CG["\[SelectionPlaceholder]"]]],
...
PasteButton["ColForm",Defer[ColumnForms[#,2]&]],
PasteButton["$ColForm",Defer[$//ColumnForms[#,2]&]],
PasteButton["$={};$ColForm",Defer[$={\[SelectionPlaceholder]};$//ColumnForms[#,2]&]],
PasteButton["T",Defer[T[\[SelectionPlaceholder],"\[Placeholder]",{\[Placeholder]}]]]
}
];*)
...
(**)
tuRuleDSolve::usage="tuRuleDSolve[var_List,indvar_List][eqn_] applies DSolve to eqn_ (maybe a List of Rules) where var_List is a list of variables that are functions of indvar_List. Does simple substitutions to transform eqn_ into form that DSolve recognizes.  PROTOTYPE. *11Jan2018*";
tuRuleDSolve[var_List,indvar_List][eqn_]:=Module[{$0=eqn,$,$s,$s1},
$s=Map[#->#[Apply[Sequence,indvar]]&,var];
$=$0/.$s/.Rule->Equal//tuDerivOps2D;
$s1=#[[2]]&/@$s;
$=DSolve[$,$s1//First,indvar];
$=$/.Map[Reverse[#]&,$s]
];
(**)
...
sub=Map[#\[Rule]Pattern[#,Btype[]] &,pvar];
tmp=tmp/.Rule[a_,b_]\[RuleDelayed]xRule[a/.sub,b];
tmp=tmp//.xRule->Rule;
tmp=tmp/.RuleDelayed[a_,b_]\[RuleDelayed]xRule[a/.sub,b];
...
tmp=tmp//.Pattern\[Rule]xPattern//.Blank\[Rule]xBlank//.BlankSequence\[Rule]xBlankSequence;
...
ExtractPositionPattern[exp_,pattern_,maxlevel_:Infinity]:=tuExtractPositionPattern[pattern,maxlevel][exp];(*Compatibility*)
(**)
